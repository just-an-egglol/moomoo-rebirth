...
export class ObjectManager {
    constructor(GameObject, gameObjects, UTILS, config, players, server) {
        ...
        // REMOVE OBJ (centralized removal, updates counts and notifies owner/viewers)
        this.removeObj = function(obj) {
            if (!obj || !obj.active) return;

            // mark inactive and cleanup pps/grids/updates
            this.disableObj(obj);

            // decrement owner's item count if applicable
            try {
                if (obj.owner && obj.group && typeof obj.group.id !== "undefined") {
                    obj.owner.changeItemCount(obj.group.id, -1);
                }
            } catch (e) {}

            // notify players who were sent this object
            for (var p = 0; p < players.length; ++p) {
                if (players[p].active && obj.sentTo[players[p].id]) {
                    players[p].send("Q", obj.sid);
                }
            }
        };
        ...
        // CHECK IF PLACABLE:
        this.checkItemLocation = function(x, y, s, sM, indx, placer) {
            for (var i = 0; i < gameObjects.length; ++i) {
