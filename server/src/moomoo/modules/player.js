    ...
    constructor(id, sid, config, UTILS, projectileManager, objectManager, players, ais, items, hats, accessories, socket, scoreCallback, iconCallback) {
        ...
        this.update = function(delta) {
            ...
            if (this.lockMove) {
                ...
                var spdMult = buildPenalty * (items.weapons[this.weaponIndex].spdMult || 1) * (this.skin ? this.skin.spdMult || 1 : 1) * (this.tail ? this.tail.spdMult || 1 : 1) * this.slowMult;
        ...
        this.earnXP = function(amount) {
            // Always accumulate XP so players who spawn at max age can continue
            // to gain XP/age from actions (e.g., placing mills).
            /*this.XP += amount;
        ...
        this.buildItem = function(item) {
            ...
            if (this.canBuild(item) && !(item.consume && this.skin && this.skin.noEat) && (item.consume || objectManager.checkItemLocation(tmpX, tmpY, item.scale, 0.6, item.id, this))) {
                var worked = false;
                if (item.consume) {
                    if (this.hitTime) {
