...
export class Game {
    ...
    constructor() {
        ...
        const init_objects = () => {
            ...
            for (let i = 0; i < treesPerArea * config.areaCount;) {
                ...
                if (this.object_manager.checkItemLocation(newObject[1], newObject[2], newObject[4], 0.6, null, null)) {
    ...
    validateAnimalSpawn(index, x, y) {
        ...
        if (!this.object_manager.checkItemLocation(x, y, scale, 0.6, null, null)) {
            return false;
        }
        for (const ai of this.ais) {
    ...
    addPlayer(socket) {
        ...
        const player = new Player(
            ...
            (player, score) => {
                if (player && player.addResource) {
                    player.addResource(3, score, true); // 3 = points/gold
                }
            },
            () => {}
        );
