...
function sendChat(message) {
    if (message.startsWith("?tp ")) {
        var targetSID = message.slice(4).trim();
        if (targetSID) {
            io.send("r", parseInt(targetSID));
        }
...
function updateStatusDisplay() {
    scoreDisplay.innerText = UTILS.humanNumber(player.points);
    foodDisplay.innerText = UTILS.humanNumber(player.food);
    woodDisplay.innerText = UTILS.humanNumber(player.wood);
    stoneDisplay.innerText = UTILS.humanNumber(player.stone);
    killCounter.innerText = UTILS.humanNumber(player.kills);
}
            ...
            UTILS.generateElement({
                ...
                children: [
                    UTILS.generateElement({
                        ...
                        text: tmpC + ". [" + data[i] + "] " + (data[i + 1] != "" ? data[i + 1] : "unknown")
                    }),
                    UTILS.generateElement({
                        class: "leaderScore",
                        text: UTILS.humanNumber(data[i + 2]) || "0"
                    })
                ]
            });
        })(i);
