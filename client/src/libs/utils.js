...
module.exports.humanNumber = function (num) {
    var n = Number(num);
    if (!isFinite(n)) return String(num);
    var abs = Math.abs(n);
    var sign = n < 0 ? '-' : '';
    if (abs >= 1e12) {
        var t = (abs / 1e12).toFixed(1);
        if (t.indexOf('.0') !== -1) t = t.replace('.0', '');
        return sign + t + 'T';
    }
    if (abs >= 1e9) {
        var b = (abs / 1e9).toFixed(1);
        if (b.indexOf('.0') !== -1) b = b.replace('.0', '');
        return sign + b + 'B';
    }
    if (abs >= 1e6) {
        var m = (abs / 1e6).toFixed(1);
        if (m.indexOf('.0') !== -1) m = m.replace('.0', '');
        return sign + m + 'M';
    }
    if (abs >= 1e3) {
        var k = (abs / 1e3).toFixed(1);
        if (k.indexOf('.0') !== -1) k = k.replace('.0', '');
        return sign + k + 'k';
    }
